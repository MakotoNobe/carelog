...
c-layout(center, :container-class="isMobile ? 'pa-0' : ''")
  ...
  .white.pa-4.mt-3
    h1.text-h3.font-weight-bold.mb-6 お礼の編集
    .mb-6
...
export default {
  ...
  mounted() {
    this.selectStaff = this.$store.getters.office.targetStaff;
    this.textData = this.$store.getters.thank.body;
  },
  methods: {
    editConfirm() {
      this.$store.dispatch("editConfirmStore", {
        staff: this.selectStaff,
        thankBody: this.textData,
        thankId: this.$route.params.id,
      });
    },
    resetVilidMessage() {
      this.errorMessages = [];
