    ...
    v-col(cols="12", md="7")
      ...
      v-card.mt-6.pa-4(:flat="isMobile", :class="{ 'pb-0': !showStaff }")
        ...
        v-row(
          ...
          v-col(cols="auto", @click="staffShow(index)")
            v-avatar(size="80")
              v-img(v-if="staff.image == null", src="/images/default.png")
              v-img(v-else, :src="staff.image")
          v-col.pt-0(@click="staffShow(index)")
            v-row
        ...
        //-           v-icon(color="admincolor") mdi-chevron-down
        ...
        v-row.darkgray--text
          ...
          v-col.py-0(cols="12")
            ...
            p.text-center.pa-1-pb-14
              p.text-decoration-none.text-center(@click="deleteAdmin") 事業所を削除する
          v-col.py-0(cols="12")
  ...
  methods: {
    ...
    async mountStaff() {
      const res = await this.$axios.get(
        `/api/offices/${this.$route.params.id}`
      );
      this.staffs = res.data.staffs;
      console.log(this.staffs);
    },
    staffShow(index) {
      this.$store.dispatch("mystaffStore", {
        staffImage: this.staffs[index].image,
        staffName: this.staffs[index].name,
        staffNameKana: this.staffs[index].name_kana,
        staffBody: this.staffs[index].body,
      });
      this.$router.push(`/staffs/${this.staffs[index].id}`);
    },
    editAdmin() {
      //   this.$store.dispatch("editAdminStore", {
      //     officeName: this.office.name,
      //     officeId: this.$route.params.id,
      //   });
      Cookies.set("office_id", this.$route.params.id);
      Cookies.set("office_name", this.office.name);
      Cookies.set("office_email", this.office.email);
      this.$router.push({
        path: `/admin/offices/edit/${Cookies.get("office_id")}`,
      });
    },
    async deleteAdmin() {
      // console.log(this.$route.params.id);
      if (confirm("本当に事業所を削除しますか？")) {
        await this.$axios
          .delete(`/api/admin/offices/${this.$route.params.id}`)
          .then(() => {
            this.$router.push("/");
          })
          .chach(() => {
...
.images__grid {
  ...
  .v-card {
    overflow: hidden;
    &.active {
      border: 3px solid var(--v-admincolor-base);
    }
    .v-image {
      transition: all 0.2s;
      position: absolute;
