...
c-layout(center)
  .white.pa-4.mt-3
    v-card.mx-auto(width="520", flat)
      h1.font-weight-bold.darkgray--text.mt-4(
        :class="(isMobile && 'text-h4') || 'text-center text-h3'"
      ) パスワードの設定
        ...
        form.form(@submit.prevent="submitResetPassword")
          ...
          .form__group
            ValidationProvider(
              ...
              p.body-1.font-weight-bold.gray--text.mb-2(
                :class="(!isMobile && 'pt-1 mt-4') || 'pt-0 mt-4'"
              ) パスワード（確認用）
              v-text-field(
                type="password",
                placeholder="半角英数字8文字以上",
...
export default {
  ...
  methods: {
    ...
    //     .then((res) => {
    //       console.log(res);
    //       this.$router.push({ path: "/signin/admin" });
    //     })
    //     .catch((res) => {
    //       console.log(res);
    //     });
    ...
    async submitResetPassword() {
      ...
      await this.$axios
        ...
        .then(() => {
          this.$router.push({ path: "/signin/office" });
        })
        .catch((res) => {
          console.log(res);
        });
